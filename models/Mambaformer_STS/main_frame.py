...
class Mid_feature(nn.Module):
    def __init__(
        ...
        if self.Space:
            if use_swin:
                ...
                self.Spa1_attn = SwinTransformerBlock(
                ...
                self.Spa1_attn = Attention(
                    dim, num_heads=num_heads,
                    attn_drop=attn_drop, proj_drop=drop, norm_layer=norm_layer, fast_attn=fast_attn,
                )

            self.Spa1_drop1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
            self.Spa1_gate = gate(dim, mlp_ratio * dim, drop=drop)
            self.Spa1_drop2 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
            self.Spa1_norm1 = norm_layer(dim)
            self.Spa1_norm2 = norm_layer(dim)
            self.Spa1_norm3 = norm_layer(dim)
        ...
        self.Sem_attn = Mamba2(  # 反向 Temporal
            d_model=dim,
            device="cuda:0",
            dt_min=0.001,
            dt_max=0.1,
        ...
        if self.Space:
            if use_swin:
                shift_size = 0 if (i % 2 == 0) else window_size // 2
                self.Spa2_attn = SwinTransformerBlock(
                    dim=dim, input_resolution=input_resolution, num_heads=num_heads, window_size=window_size,
                    shift_size=shift_size, drop=drop, attn_drop=attn_drop,
                )
            else:
                self.Spa2_attn = Attention(
                    dim, num_heads=num_heads,
                    attn_drop=attn_drop, proj_drop=drop, norm_layer=norm_layer, fast_attn=fast_attn,
                )

            self.Spa2_drop1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
            self.Spa2_gate = gate(dim, mlp_ratio * dim, drop=drop)
            self.Spa2_drop2 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
            self.Spa2_norm1 = norm_layer(dim)
            self.Spa2_norm2 = norm_layer(dim)
            self.Spa2_norm3 = norm_layer(dim)
    ...
    def forward(self, x):
        ...
        if self.Space:
            x = x.reshape(-1, n, c)
            x = x + self.Spa1_drop1(self.Spa1_attn(self.Spa1_norm1(x)))
            x = x + self.Spa1_drop2(self.Spa1_gate(self.Spa1_norm2(x)))
            x = self.Spa1_norm3(x)
            x = x.view(b, t, n, c)  #
        ...
        gate_weights = self.temporal_fusion_gate(combined)
        ...
        x = self.Tem_norm3(x)
        x = x.view(b, n, t, c).transpose(1, 2)
        if self.Space:
            x = x.reshape(-1, n, c)
            x = x + self.Spa2_drop1(self.Spa2_attn(self.Spa2_norm1(x)))
            x = x + self.Spa2_drop2(self.Spa2_gate(self.Spa2_norm2(x)))
            x = self.Spa2_norm3(x)
            x = x.view(b, t, n, c)
        return x.contiguous()
