...
from models.layers.block import Mlp, get_sinusoid_encoding_table
from models.Mambaformer_TSS.main_frame import Early_feature, Mid_feature
from utils.str_bool import str2bool

warnings.filterwarnings("ignore", category=UserWarning)


class Mambaformer_TSS(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.hparams.mtm and self.training and ((torch.rand(1) <= self.hparams.p_use_mtm).all()):
            token_mask = (
                (
                    ...
                    .type_as(patches_embed)  # mlp common way to ensure tensor type consistency and device consistency
        ...
        x = patches_embed + self.pos_embed.to(patches_embed.device)
        ...
        images_out = (
            x_out.reshape(
                B, self.total_T, self.input_resolution, self.input_resolution, self.hparams.n_out,
                self.hparams.patch_size, self.hparams.patch_size,
            )
...
if __name__ == "__main__":
    arg = Mambaformer.add_model_specific_args()
    args = arg.parse_args()
    model = Mambaformer_TSS(args)
