...
class Mid_feature(nn.Module):
    def __init__(
        ...
        self.Tem1_attn = Mamba2(
            d_model=dim, device="cuda:0", dt_min=0.001, dt_max=0.1, d_state=256,
            d_conv=4, expand=2, headdim=64, D_has_hdim=True, norm_before_gate=True
        )
        self.Sem1_attn = Mamba2(
            d_model=dim, device="cuda:0", dt_min=0.001, dt_max=0.1, d_state=256,
            d_conv=4, expand=2, headdim=64, D_has_hdim=True, norm_before_gate=True
        )
        self.Tem1_drop1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.Tem1_gate = gate(dim, mlp_ratio * dim, drop=drop)
        self.Tem1_drop2 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.Tem1_norm1 = norm_layer(dim)
        self.Tem1_norm2 = norm_layer(dim)
        self.Tem1_norm3 = norm_layer(dim)
        ...
        self.Tem2_attn = Mamba2(
            d_model=dim, device="cuda:0", dt_min=0.001, dt_max=0.1, d_state=256,
            d_conv=4, expand=2, headdim=64, D_has_hdim=True, norm_before_gate=True
        )
        self.Sem2_attn = Mamba2(
            d_model=dim, device="cuda:0", dt_min=0.001, dt_max=0.1, d_state=256,
            d_conv=4, expand=2, headdim=64, D_has_hdim=True, norm_before_gate=True
        )
        self.Tem2_drop1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.Tem2_gate = gate(dim, mlp_ratio * dim, drop=drop)
        self.Tem2_drop2 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.Tem2_norm1 = norm_layer(dim)
        self.Tem2_norm2 = norm_layer(dim)
        self.Tem2_norm3 = norm_layer(dim)
    ...
    def forward(self, x):
        ...
        x_fwd1 = self.Tem1_attn(self.Tem1_norm1(x))
        x_bwd1 = self.Sem1_attn(self.Tem1_norm1(x).flip(1)).flip(1)
        gw1 = self.fusion_gate1(torch.cat([x_fwd1, x_bwd1], dim=-1))
        out1 = gw1 * x_fwd1 + (1 - gw1) * x_bwd1
        x = x + self.Tem1_drop1(out1)
        x = x + self.Tem1_drop2(self.Tem1_gate(self.Tem1_norm2(x)))
        x = self.Tem1_norm3(x)
        ...
        if self.Space:
            ...
            x = self.Spa_norm3(x)
        x = x.view(b, t, n, c).transpose(1, 2).reshape(-1, t, c)
        x_fwd2 = self.Tem2_attn(self.Tem2_norm1(x))
        x_bwd2 = self.Sem2_attn(self.Tem2_norm1(x).flip(1)).flip(1)
        gw2 = self.fusion_gate2(torch.cat([x_fwd2, x_bwd2], dim=-1))
        out2 = gw2 * x_fwd2 + (1 - gw2) * x_bwd2
        x = x + self.Tem2_drop1(out2)
        x = x + self.Tem2_drop2(self.Tem2_gate(self.Tem2_norm2(x)))
        x = self.Tem2_norm3(x)
        return x.view(b, n, t, c).transpose(1, 2).contiguous()
