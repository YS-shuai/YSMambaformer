...
class conv_blockv2(nn.Module):
    def __init__(self, in_channel, out_channel, resolution_h,resolution_w, field_size, bias=True):
        super().__init__()
        if field_size == 1:
            self.Conv = nn.Sequential(
                nn.Conv2d(in_channel, out_channel, kernel_size=1, padding=0, bias=bias,),
                nn.LayerNorm([out_channel, resolution_h,resolution_w]),
            )
        elif field_size == 3:
            self.Conv = nn.Sequential(
                nn.Conv2d(in_channel, out_channel, kernel_size=1, padding=0, bias=bias,),
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, resolution_h,resolution_w, dim, norm_layer=nn.LayerNorm):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = x.view(b*t, self. resolution_h, self. resolution_w, c)
