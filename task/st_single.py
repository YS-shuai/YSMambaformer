...
class SpatioTemporal_single(pl.LightningModule):
    def __init__(self, model: nn.Module, hparams: argparse.Namespace, logger=None):
        ...
        self.vision = Vision(save_dir=self.pred_dir, target_length=hparams.target_length, data_name=self.hparams.data_name)
    @staticmethod
    def add_task_specific_args(
        ...
        parser.add_argument("--data_name", type=str, default="human")
        ...
        # Context and target length for temporal model. mlp temporal model use a context period to learn the temporal dependencies and predict the target period.
    ...
    def configure_optimizers(self):
        ...
        for optimizer, sched in zip(optimizers, self.hparams.optimization["lr_shedule"]):
            if sched["name"] == 'OneCycleLR':
                # OneCycleLR 需要 total_steps 参数
                steps_per_epoch = len(self.trainer.datamodule.train_dataloader())
                total_steps = steps_per_epoch * self.trainer.max_epochs
                scheduler = OneCycleLR(
                    optimizer,
                    max_lr=sched["args"]["max_lr"],
                    total_steps=total_steps,
                    final_div_factor=sched["args"].get("final_div_factor", 0.0001)
                )
            elif sched["name"] == 'Cosine':
                scheduler = CosineAnnealingLR(
                            optimizer=optimizer,
                            T_max=sched["args"]["t_initial"],
                            eta_min=sched["args"]["lr_min"],
                )
            else:
                # 其他学习率调度器
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # if self.current_epoch % 10 == 9 and batch_idx < 10:
