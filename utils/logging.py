...
CMAP_NDVI = clr.LinearSegmentedColormap.from_list('ndvi',
                                                  ["#cbbe9a", "#fffde4", "#bccea5", "#66985b", "#2e6a32", "#123f1e",
                                                   "#0e371a", "#01140f", "#000d0a"], N=256)
...
def text_phantom(text, width):  # TODO move to generic function
    ...
    bbox = pil_font.getbbox(text)  # 获取文本的边界框
    text_width, text_height = bbox[2] - bbox[0], bbox[3] - bbox[1]  # 计算宽度和高度
...
def veg_colorize(data, mask=None, clouds=None,
                 setting="en21x"):  # TODO move to generic function or take from earthnet.tk
    # mask has 1 channel
    ...
    if clouds is None:
        return torch.as_tensor(cmap(in_data)[:, :3], dtype=data.dtype, device=data.device).reshape(t, h, w, 3).permute(
            0, 3, 1, 2)
    else:
        out = torch.as_tensor(cmap(in_data)[:, :3], dtype=data.dtype, device=data.device).reshape(t, h, w, 3).permute(0,
                                                                                                                      3,
                                                                                                                      1,
                                                                                                                      2)
        return torch.stack([torch.where(clouds, out[:, 0, ...],
                                        torch.zeros_like(out[:, 0, ...], dtype=data.dtype, device=data.device)),
                            torch.where(clouds, out[:, 1, ...],
                                        torch.zeros_like(out[:, 1, ...], dtype=data.dtype, device=data.device)),
                            torch.where(clouds, out[:, 2, ...],
                                        0.1 * torch.ones_like(out[:, 2, ...], dtype=data.dtype, device=data.device))],
                           dim=1)
...
def log_viz(tensorboard_logger, viz_data, batch, batch_idx, current_epoch, setting="en21x"):
    ...
    for i, (preds, scores) in enumerate(viz_data):
        for j in range(preds.shape[0]):
            if setting == "en21x":
                ndvi = veg_colorize(preds[j, ...].squeeze(), mask=lc[j, ...].repeat(preds.shape[1], 1, 1),
                                    setting=setting)
                text = f"Cube: {scores[j]['name']} Veg Score: {scores[j]['veg_score']:.4f}"
                grid = torchvision.utils.make_grid(ndvi, nrow=nrow)
                text = torch.tensor(text_phantom(text, width=grid.shape[-1]), dtype=torch.float32,
                                    device=targs.device).type_as(grid).permute(2, 0, 1)
                ...
                tensorboard_logger.add_image(f"Cube: {batch_idx * preds.shape[0] + j} RGB Preds, Sample: {i}", grid,
                                             current_epoch)
                ndvi = veg_colorize(
                    (preds[j, :, 3, ...] - preds[j, :, 2, ...]) / (preds[j, :, 3, ...] + preds[j, :, 2, ...] + 1e-6),
                    mask=None if "landcover" not in batch else lc[j, ...].repeat(preds.shape[1], 1, 1), setting=setting)
                grid = torchvision.utils.make_grid(ndvi, nrow=nrow)
            else:

                ndvi = veg_colorize(preds[j, ...].squeeze(),
                                    mask=None if "landcover" not in batch else lc[j, ...].repeat(preds.shape[1], 1, 1),
                                    setting=setting)
            ...
            ndvi = (preds[j, :, 3, ...] - preds[j, :, 2, ...]) / (
                        preds[j, :, 3, ...] + preds[j, :, 2, ...] + 1e-6) if setting == "en21-std" else preds[
                j, ...].squeeze()
            ...
            if i == 0:
                if setting == "en21x":
                    rgb = torch.cat([targs[j, :, 3, ...].unsqueeze(1) * 10000, targs[j, :, 2, ...].unsqueeze(1) * 10000,
                                     targs[j, :, 1, ...].unsqueeze(1) * 10000], dim=1)
                    grid = torchvision.utils.make_grid(rgb, nrow=nrow, normalize=True, value_range=(0, 5000))
                    tensorboard_logger.add_image(f"Cube: {batch_idx * preds.shape[0] + j} RGB Targets", grid,
                                                 current_epoch)

                    ndvi = veg_colorize(targs[j, :, 0, ...], mask=lc[j, ...].repeat(targs.shape[1], 1, 1),
                                        clouds=masks[j, :, 0, ...], setting=setting)
                    ...
                    tensorboard_logger.add_image(f"Cube: {batch_idx * preds.shape[0] + j} RGB Targets", grid,
                                                 current_epoch)
                    ndvi = veg_colorize((targs[j, :, 3, ...] - targs[j, :, 2, ...]) / (
                                targs[j, :, 3, ...] + targs[j, :, 2, ...] + 1e-6),
                                        mask=None if "landcover" not in batch else lc[j, ...].repeat(targs.shape[1], 1,
                                                                                                     1),
                                        clouds=masks[j, :, 0, ...] if masks is not None else None, mode="ndvi")
                else:  # kndvi
                    ndvi = veg_colorize(targs[j, :, 0, ...],
                                        mask=None if "landcover" not in batch else lc[j, ...].repeat(targs.shape[1], 1,
                                                                                                     1), clouds=None,
                                        setting=setting)
                grid = torchvision.utils.make_grid(ndvi, nrow=nrow)
                tensorboard_logger.add_image(f"Cube: {batch_idx * preds.shape[0] + j} NDVI Targets", grid,
                                             current_epoch)
