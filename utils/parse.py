...
import yaml
import warnings
from dataset.data_selection import SETTINGS, METRIC_CHECKPOINT_INFO
from models.selection_model import MODELS


def parse_setting(setting_file, track=None):
    setting_file = Path(setting_file)

    # decompose the path of the setting_file (in ConvLSTM/)
    try:
        if "grid" == setting_file.parts[-2]:
            ...
            setting, architecture, feature, config = setting_file.parts[
                                                     -4:]  # example: setting: en21x, architecture: local-rnn, feature: arch, config: base.yaml
    ...
    if "Logger" in setting_dict:
        if "save_dir" in setting_dict["Logger"]:
            ...
            if (len(list(Path(save_dir).parts)) < 2) or ((Path(save_dir).parts[-2] != setting_dict["Setting"]) and (
                    Path(save_dir).parts[-1] != setting_dict["Architecture"])):
                setting_dict["Logger"]["save_dir"] = str(
                    Path(save_dir) / setting_dict["Setting"] / setting_dict["Architecture"])
        else:
            setting_dict["Logger"]["save_dir"] = str(
                Path("experiments/") / setting_dict["Setting"] / setting_dict["Architecture"])
    ...
    setting_dict["Data"]["fp16"] = (
                setting_dict["Trainer"]["precision"] == 16)  # binary floating-point computer number format
    ...
    if setting_dict["Architecture"] in ["channel-u-net", "local-rnn", "rnn", "context-convlstm", "u-net-convlstm",
                                        "dumby-mlp", "convlstm-lstm"]:
    ...
    if setting_dict["Setting"] in ["greenearthnet"]:
